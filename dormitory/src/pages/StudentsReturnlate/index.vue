<template>
  <div class="all">
    ...
    <div v-if="switchs" class="footer">
      <h2>记录详情</h2>
      <div class="footer_2">
        <a-table :columns="columns1" :data-source="dataSource" :style="{ width: '100%' }" :pagination="false">
          <template #bodyCell="{ column, record }">
            ...
            <template v-if="column.key === 'checkTime'">
              <span>{{ record.time.slice(0, 2) < 22 ? '是' : '否' }}</span>
            </template>
          </template>
        </a-table>
      </div>
...
<script lang="ts" setup>
  ...
  import { floormanagement, dormitoryInfos, late } from '../../api/test/index';
  import { DataItem } from '@/layout/admin';
  import type { Dayjs } from 'dayjs';
  import { columns1 } from '@/layout/column';
  ...
  const getdata = () => {
    late(userId.value).then((res: any) => {
